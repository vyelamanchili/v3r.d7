		...
		$(this).each(function(){
			...
			function loadRegionData(region){
				...
				map.load(url, {}, function(){
					//place bullets
					var dividtodisplay = '#infodivclick';
					$(this).children('a.bullet').each(function(){
						...
						$(this).css({left: addpx(Number(coords[0]) - rempx(settings.bulletWidthOffset)), top: addpx(Number(coords[1]) - rempx(settings.bulletHeightOffset))})
							   ...
							   .click(function(){showPopup($(this).attr('id'));})
								 .hover(function(){$(this).append('<div class="infodivtext">Click for more information</div>');	} , 
												function(){$("div").remove('.infodivtext'); } )
							   .fadeIn('fast');
					});
			...
			function addZoom(region){
				$('<img />').addClass(settings.zoomClass)
					.attr({
						...
						title: "Click here to zoom in.",
						alt: "Click here to zoom in.",
						id: region.id
					}).css({
						position: 'absolute',
						width: region.width,
				...
				/*	$(region.id).hover(function(){map.append('<div class="infodivtext">Click here to zoom in.</div>');	} , function(){$("div").remove('.infodivtext'); } )	;*/
			}
